...
[data-testid="stButton"] button {
    border-radius: 999px;
    border-color: rgba(54, 125, 138, .55);
}
...
def run_live_check(prefix_filter=None):
    if prefix_filter:
        tasks = [(prefix_filter, second) for second in ALPHABET]
    else:
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        futures = [
            executor.submit(check_one_code, first, second)
...
with left:
    button_label = (
        "Run live check for selected prefix"
        if scan_mode == "Selected prefix only"
        else "Run live check for all prefixes"
    )
    ...
    if st.button(button_label, use_container_width=True):
        ...
        if prefix_filter:
            st.info(f"Running live check for prefix {selected_prefix}.")
            ...
            st.info("Running live check for all prefixes.")
...
<div class="stats-grid">
  ...
  <div class="stat-card">
    ...
    <div class="stat-value">{working_links}</div>
  </div>
  <div class="stat-card">
    <div class="stat-label">Dead / Unknown</div>
    <div class="stat-value">{dead_links}</div>
  </div>
</div>
""",
    unsafe_allow_html=True,
...
st.markdown(
    f'<div class="section-label">{section_title}</div>',
    unsafe_allow_html=True,
)
...
for group in groups:
    ...
    for second in ALPHABET:
        ...
        if is_working:
            ...
            tiles.append(
                f'<a class="link-tile link-working" href="{safe_url}" '
                f'target="_blank" rel="noopener">{safe_code}</a>'
            )
        else:
            tiles.append(
                f'<span class="link-tile link-dead">{safe_code}</span>'
            )
